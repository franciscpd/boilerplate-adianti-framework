...
class SystemSqlLogService implements AdiantiLoggerInterface
{
    ...
    public function write($message)
    {
        ...
        $pattern = "/(UPDATE (.*?) SET)|(INSERT INTO (.*?) \()|(FROM (.*?) WHERE)/";
        preg_match($pattern, $message, $matches);
        $table_name = end($matches);
        
        $is_table_log = in_array($table_name, array('system_change_log', 'system_sql_log', 'system_access_log', 'system_request_log'));
        $is_command_log = in_array(substr($message,0,6), array('INSERT', 'UPDATE', 'DELETE'));
        
        // avoid log of log
        if (!$is_table_log AND $is_command_log AND isset($table_name))
        {
